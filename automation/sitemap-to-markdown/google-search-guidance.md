...
2. Use the existing webpage as the primary source. The workflow should extract the page title, headings, visible text, metadata, canonical URL, and links, then prepare an editorial draft only where it can add clear user value.
...
5. Preserve accurate authorship and review context. The Markdown front matter should identify the responsible editorial team and the human reviewer when a page requires additional verification.
...
YAML front matter should contain: title, description, canonical, source_url, slug, language, page_type, target_audience, primary_topic, suggested_schema_type, published_at, source_fetched_at, review_status, and reviewer_notes.
